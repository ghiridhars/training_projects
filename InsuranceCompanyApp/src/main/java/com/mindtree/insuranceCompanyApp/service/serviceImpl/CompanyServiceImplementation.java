...
//			cList = cList.stream().filter(x -> x.getPolicies().size() >= 3).collect(Collectors.toList());
		...
		Company cL[] =new Company[cList.size()];
		int i = 0;
		while(itr.hasNext()) {
			cL[i] = itr.next();
		}
	...
	@Cacheable(value="cList",key = "#id",condition = "#id>=4")
	@Override
	public CompanyDto getCompany(int id) throws CompanyServiceException {
		Company c= null;
		try {
			Thread.sleep(900);
			c = companyRepo.findById(id).orElseThrow(() -> new InvalidCompanyException("Company Not Found"));
